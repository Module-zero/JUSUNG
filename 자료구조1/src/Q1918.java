...
// 닫힌 괄호를 만났을 때 스택에서 열린 괄호가 나올때까지 꺼내기만하면 되는 것이었다.
// 스택의 성질로 인해 어차피 가장 먼저 만나는 열린괄호가 닫힌 괄호와 짝일 수 밖에 없을 것이기 때문이다
// 괄호 처리를 위한 재귀 사용은 실패에 가까운것 같다
// 확실치는 않지만 스택으로 해결하는 문제는 어떤 값을 스택에 넣고 어떤 값을 스택에 안넣는지를 구분하는 것이 우선적인 접근인 것 같다.
public class Q1918 {
